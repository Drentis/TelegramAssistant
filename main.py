...
async def weather_scheduler(bot: Bot):
    ...
    user_tz_offsets = {}  # {user_id: timezone_offset} - кэш часовых поясов пользователей
    ...
    while True:
        ...
        for user in users:
            ...
            # Получаем часовой пояс пользователя (из кэша или вычисляем)
            tz_offset = user_tz_offsets.get(user_id)
            if tz_offset is None:
                # Используем системное время пользователя как приближение
                # Это работает если бот и пользователь в одном часовом поясе
                # Для точного определения нужно хранить timezone в БД
                tz_offset = datetime.now().astimezone().utcoffset().total_seconds()
                user_tz_offsets[user_id] = tz_offset
