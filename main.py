...
def get_edit_keyboard(items: list, list_type: str, category: str = None):
    ...
    for item in items:
        ...
        if list_type == "shopping":
            ...
            # Для todo, study и ideas - используем .get() для безопасного доступа
            if list_type == "ideas":
                item_text = item.get('idea', 'Без названия')
            else:
                # Для todo и study
                item_text = item.get('task', 'Без названия')
    ...
    if len(parts) >= 1:
        ...
        if list_type == "shopping":
            ...
            try:
                ...
                text = "📋 **Список дел:**\n\n"
                for item in items:
                    task = item.get('task', 'Без названия') if hasattr(item, 'get') else getattr(item, 'task', 'Без названия')
                    text += f"❌ {task}\n"
