...
from magic_filter import F as MagicF
...
def get_items_keyboard(items: list, list_type: str, category: str = None):
    ...
    """
    # Формируем callback_data без пустой строки в конце
    if category:
        edit_callback = f"edit_{list_type}_{category}"
    else:
        edit_callback = f"edit_{list_type}"
    
    buttons = [
        [InlineKeyboardButton(text="✏️ Редактировать список", callback_data=edit_callback)]
    ]
...
def get_edit_keyboard(items: list, list_type: str, category: str = None):
    ...
    for item in items:
        ...
        if list_type == "shopping":
            ...
            # Для todo, study и ideas - используем доступ через [] для aiosqlite.Row
            if list_type == "ideas":
...
async def cmd_help(message: types.Message):
    ...
    await message.answer(
        ...
        f"/update — Обновить бота (админ)\n"
        f"/delete — Удалить бота (админ)",
        reply_markup=get_main_keyboard(settings),
        parse_mode="Markdown"
    )
...
async def cmd_delete_confirm(message: types.Message, state: FSMContext):
    ...
    # Очищаем состояние и останавливаем бота
...
async def stop_bot():
    """Остановка бота после удаления."""
    await asyncio.sleep(2)
    import sys
    sys.exit(0)


async def handle_shopping_button(message: types.Message):
    """
...
async def handle_edit_list_callback(callback: types.CallbackQuery):
    ...
    # Отладка: логируем callback data
    user_id = callback.from_user.id
    callback_data = callback.data
    print(f"[DEBUG] ========== handle_edit_list_callback ==========")
    ...
    if len(parts) >= 1:
        list_type = parts[0]
        # category может быть None если parts всего 1 (например "todo")
        category = parts[1] if len(parts) > 1 else None
        
        # Отладка: логируем list_type и category
        print(f"[DEBUG] list_type: {list_type}")
        ...
        if list_type == "shopping":
            ...
            try:
                ...
                print(f"[DEBUG] edit_text failed for shopping: {e}")
                await callback.message.answer(text, reply_markup=keyboard, parse_mode="Markdown")
        elif list_type == "todo":
            print(f"[DEBUG] Processing todo list...")
            try:
                items = await db.get_todo_items(callback.from_user.id)
                print(f"[DEBUG] Got {len(items)} todo items")
                
                # Проверяем первый элемент если есть
                if len(items) > 0:
                    first_item = items[0]
                    print(f"[DEBUG] First item type: {type(first_item)}")
                    print(f"[DEBUG] First item keys: {first_item.keys() if hasattr(first_item, 'keys') else 'N/A'}")
                    print(f"[DEBUG] First item: {dict(first_item) if hasattr(first_item, 'keys') else first_item}")
                
                text = "📋 **Список дел:**\n\n"
                for item in items:
                    task = item['task'] if 'task' in item.keys() else 'Без названия'
                    text += f"❌ {task}\n"
                text += f"\nВсего: {len(items)}\n\n✏️ _Нажимайте на задачи, чтобы удалить их_"
                
                keyboard = get_edit_keyboard(items, "todo")
                print(f"[DEBUG] Created keyboard with {len(keyboard.inline_keyboard)} buttons")

                try:
                    await callback.message.edit_text(text, reply_markup=keyboard, parse_mode="Markdown")
                    print(f"[DEBUG] edit_text succeeded for todo")
                except Exception as e:
                    # Если не удалось отредактировать, отправляем новое сообщение
                    print(f"[DEBUG] edit_text failed for todo: {e}")
                    print(f"[DEBUG] Trying to send new message...")
                    await callback.message.answer(text, reply_markup=keyboard, parse_mode="Markdown")
                    print(f"[DEBUG] answer succeeded for todo")
            except Exception as e:
                print(f"[DEBUG] CRITICAL ERROR in todo processing: {e}")
                import traceback
                traceback.print_exc()
            ...
            try:
                ...
                # Если не удалось отредактировать, отправляем новое сообщение
                print(f"[DEBUG] edit_text failed for study: {e}")
                await callback.message.answer(text, reply_markup=keyboard, parse_mode="Markdown")
        elif list_type == "ideas":
            ...
            try:
                ...
                # Если не удалось отредактировать, отправляем новое сообщение
                print(f"[DEBUG] edit_text failed for ideas: {e}")
                await callback.message.answer(text, reply_markup=keyboard, parse_mode="Markdown")

    print(f"[DEBUG] ========== End of callback handler ==========")
    await callback.answer()
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params, timeout=10) as response:
                if response.status == 200:
                    ...
                    max_temp = -50
                    min_temp = 50
                    # Получаем часовой пояс города из первого элемента (timezone в секундах)
                    # dt_txt в API всегда в UTC, поэтому используем timezone из response
                    city_timezone = data.get("city", {}).get("timezone", 0)

                    for item in data["list"][:8]:  # 8 интервалов по 3 часа = 24 часа
                        # dt_txt в UTC, конвертируем в локальное время города
                        dt_utc = datetime.fromisoformat(item["dt_txt"])
                        dt_local = dt_utc + timedelta(seconds=city_timezone)
                        weather_id = item["weather"][0]["id"]
                        temp = item["main"]["temp"]

                        # ID погоды: 2xx=гроза, 3xx=морось, 5xx=дождь
                        if weather_id >= 200 and weather_id < 600:
                            rain_hours.append(dt_local.strftime("%H:%M"))
                    ...
                    return {
                        ...
                        "timezone": city_timezone  # Возвращаем часовой пояс города
                    }
                else:
                    return {"error": "Ошибка API"}
    except Exception as e:
...
async def handle_admin_update_bot_callback(callback: types.CallbackQuery):
    ...
    try:
        ...
        if not os.path.exists(git_dir):
            ...
            subprocess.run(['git', 'remote', 'add', 'origin', remote_url], cwd=bot_dir, capture_output=True, check=True)
            subprocess.run(['git', 'fetch', 'origin'], cwd=bot_dir, capture_output=True, timeout=30, check=True)
            subprocess.run(['git', 'checkout', '-f', 'master'], cwd=bot_dir, capture_output=True, check=True)
            subprocess.run(['git', 'reset', '--hard', 'origin/master'], cwd=bot_dir, capture_output=True, timeout=30, check=True)

            result_stdout = "✓ Репозиторий инициализирован\n✓ Файлы обновлены"
            result_stderr = ""
        else:
            # Принудительное обновление через fetch + reset (игнорирует локальные изменения)
            subprocess.run(['git', 'fetch', 'origin'], cwd=bot_dir, capture_output=True, timeout=30, check=True)
            result = subprocess.run(
        ...
        if result_stderr and 'error' in result_stderr.lower():
            ...
            await callback.message.answer(
                "✅ **Бот обновлён!**\n\n"
                "Обновления применены. **Перезапустите бота** для применения изменений.\n\n"
                f"📝 **Что изменилось:**\n```{result_stdout[:1000] if result_stdout else 'Файлы загружены'}```",
                parse_mode="Markdown"
            )
...
async def handle_weather_time_input(message: types.Message, state: FSMContext):
    ...
    # Проверяем формат времени (ЧЧ:ММ или Ч:ММ) - принимаем оба формата
    time_match = re.match(r'^([0-9]{1,2}):([0-5][0-9])$', time_text)
    if not time_match:
        await message.answer(
            "❌ Неверный формат времени.\n\n"
            "Используйте формат ЧЧ:ММ (например, 6:00, 06:00, 8:30, 20:00)\n\n"
            "Попробуйте ещё раз:",
            parse_mode="Markdown"
        )
        return
    ...
    # Проверяем корректность часов (0-23)
    if hour > 23:
        await message.answer(
            "❌ Неверный формат времени.\n\n"
    ...
    # Нормализуем время до формата ЧЧ:ММ (с ведущим нулём для хранения)
    ...
    await message.answer(
        ...
        f"Теперь прогноз погоды будет приходить ежедневно в {normalized_time} по вашему местному времени.",
        parse_mode="Markdown"
    )
    # Возвращаем настройки
    settings = await db.get_category_settings(message.from_user.id)
...
async def weather_scheduler(bot: Bot):
    """Планировщик погоды (каждый день в настроенное время по часовому поясу города)."""
    last_sent = {}  # {user_id: date} - отслеживаем, кому уже отправили сегодня
    city_timezones = {}  # {city: timezone_offset} - кэш часовых поясов городов
    ...
    while True:
        ...
        for user in users:
            ...
            # Получаем часовой пояс города из кэша или API
            tz_offset = city_timezones.get(city)
            if tz_offset is None:
                # Запрашиваем часовой пояс города через OpenWeatherMap API
                weather_data = await get_weather(city)
                if weather_data.get("success"):
                    tz_offset = weather_data.get("timezone", 0)
                    city_timezones[city] = tz_offset
                else:
                    # Если не удалось получить часовой пояс, пропускаем пользователя
                    continue

            # Вычисляем локальное время в городе пользователя
...
async def main():
    ...
    dp.message(Command("delete", magic=MagicF.args == "confirm"))(cmd_delete_confirm)
